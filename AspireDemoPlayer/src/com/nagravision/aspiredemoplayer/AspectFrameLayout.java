...
public class AspectFrameLayout extends FrameLayout
{
    ...
    public void setAspectRatio(double aspectRatio)
    {
        ...
        Log.d(TAG, "Setting aspect ratio to " + aspectRatio + " (was "
            + mTargetAspect + ")");
    ...
    {
        Log.d(TAG, "onMeasure target=" + mTargetAspect + " width=["
            + View.MeasureSpec.toString(mMaxWidth) + "] height=["
            + View.MeasureSpec.toString(heightMeasureSpec) + "]");

        // Target aspect ratio will be < 0 if it hasn't been set yet. In that
        // case,
        // we just use whatever we've been handed.
        if (mTargetAspect > 0)
        {
            int initialWidth = View.MeasureSpec.getSize(mMaxWidth);
            ...
            {
                // We're very close already. We don't want to risk switching
                // from e.g. non-scaled
                // 1280x720 to scaled 1280x719 because of some floating-point
                // round-off error,
