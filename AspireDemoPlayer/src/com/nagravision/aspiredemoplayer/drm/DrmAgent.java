...
public final class DrmAgent {
	...
	public boolean registerDevice(){
		...
		for(;;){
			DrmInfoRequest drmInfoRequest = new DrmInfoRequest(DrmInfoRequest.TYPE_REGISTRATION_INFO,"video/mp4");
			...
			if(DrmManagerClient.ERROR_NONE != mDrmMgrClt.processDrmInfo(drmInfo)){
	...
	public boolean acquireDrmInfoRights(String xContentId, String xMimeType)	{
		...
		try 
		{
			for(;;){
				...
				/*ContentValues values = mDrmMgrClt.getConstraints(xVideoLocation, DrmStore.Action.PLAY);
				if(null == values){
